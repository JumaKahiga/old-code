...
if (isset($_FILES['documentation']['name']))
{
	...
	if($typeok)
	{
		...
		queryMysql("insert into unitDocuments (name,projectID,location) values ('$name','$projectId','$saveto')");
		echo "<script>location.replace('getUnit.php?id=$projectId')</script>";
	}
	else
	{
